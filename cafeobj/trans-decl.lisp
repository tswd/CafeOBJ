...
(defun process-axiom-form (decl &rest ignore)
  ...
  (let (type
    ...
    (when (and (not (equal (first lhs) "("))
	       ...
	       (equal (third lhs) "]")
	       (equal (fourth lhs) ":"))
      (setf labels (list (intern (string (second lhs)))))
...
(defun parse-module-elements (s &rest ignore)
  ...
  (let ((body nil))
    (dolist (e s)
      (setf body (nconc body (parse-module-element e))))
    body))
...
(defun parse-module-element (e &rest ignore)
  ...
  (case-equal (car e)
    ...
    ("bsort" (list (process-bsort-declaration e)))
    ("[" (process-sort-and-subsort-form e))
    (("hidden" "*") (process-hidden-sort-form e))

    ;; OPERATOR DECLARATION.
    ("op"
     (list (process-operator-declaration-form e)))
    ("ops"
     (process-operators-declaration-form e))
    ("bop"
     (list (process-operator-declaration-form e)))
    ("bops"
     (process-boperators-declaration-form e))

    ;; predicate
    ("pred"
     (list (process-predicate-declaration-form e)))
    ...
    ;; OPERATOR-ATTRIBUTES.
    (("opattr" "attr" "attrs")
     (list (process-opattr-declaration-form e)))
	
    ;; RECORD DECLARATION.
    (("rec" "record")
     (list (process-record-declaration-form e)))

    ;; CLASS DECLARATION.
    (("cls" "class")
     (list (process-class-declaration-form e)))

    ;; LET Construct. 
    ("let" (list (process-let-declaration-form e)))

    ;; Macro
    ("#define" (list (process-macro-declaration-form e)))
    
    ;; Axioms
    (("eq" "cq" "ceq" "rule" "rl" "crl" "crule" "trans" "ctrans" "trns" "ctrns"
	   "beq" "bceq" "brule" "brl" "bcrule" "bcrl" "btrans" "btrns"
	   "bctrans" "bctrns")
     (list (process-axiom-form e)))

    ;; VARIABLE declaration.
    (("var" "vars")
     (list (process-variable-declaration-form e)))
	
    (("pvar" "pvars")
     (list (process-pseud-variable-declaration-form e)))
    
    #+:bigpink
    ;; FOPL
    (("fax" "bfax" "ax" "bax" "frm" "bfrm")
     (list (pignose-process-fax-proc e)))
    #+:bigpink
    (("goal" "bgoal")
     (list (pignose-process-goal-proc e)))
    
    #||
    ("vars-of"				; NOT YET
     (list (process-vars-of-declaration-form e)))
    ||#
    ;; IMPORTATIONS.
    (("imports" "import")
     (let ((body nil)
    ...
    (("pr" "protecting" "ex" "extending" "us" "using" "inc" "including")
     (process-importation-form e))

    ;; Comments.
    ;; must prepare ....
    (("--" "**") nil)			; do nothing
    ;; should delay the output.
    ("-->" (list (%dyna-comment* (cons "--" (cdr e)))))
    ("**>" (list (%dyna-comment* (cons "**" (cdr e)))))
    ...
    (("sig" "signature")
     (let ((body nil)
       ...
       (unless (equal s-body "}")
         (dolist (elt s-body)
	   (unless (equal elt "}")
	     (setf body (nconc body (parse-module-element elt))))))
       body))
