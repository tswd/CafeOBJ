...
;;; $Id: match-system.lisp,v 1.4 2007-04-03 06:46:14 sawada Exp $
(in-package :chaos)
#|=============================================================================
				  System:CHAOS
				 Module:e-match
	     ...
	     (let ((cval (variable-image (cdr res) t1)))
		   ...
		   (progn
		     ...
		     (with-output-simple-msg ()
		       (format t "* rec!match-dec: t1=")
		       (term-print t1)
		       (print-next)
		       (format t " t2 = ")
		       (term-print t2)
		       (print-next)
		       (print-substitution (cdr res)))
		     ||#
...
(defun match-insert-if-coherent-with (new-env test-env new-sys eq-list
